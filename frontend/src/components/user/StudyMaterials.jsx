...
const StudyMaterials = () => {
  ...
  const handleDownload = async (id, title) => {
    try {
      ...
      setError(
        err.response?.data?.message || "Failed to download study material"
      );
    }
  };

  const handleView = async (id, _title) => {
    try {
  ...
  return (
    <div className="space-y-6">
      {showTest && selectedMaterial && (
        <TestInterface
          studyMaterialId={selectedMaterial.id}
          studyMaterialTitle={selectedMaterial.title}
          onClose={() => {
            setShowTest(false);
